...
| `components__misc__CustomShortcut.qml.patch` | Rebinds `GlobalShortcut.appid` from `caelestia` to `cortetsu`, the public Hyprland IPC contract | `cortetsu/modules/CortetsuShortcut.qml` (first-party shortcut, same `appid: "cortetsu"` contract), `scripts/features/test-shortcut-namespace.py` (asserts the patch literally) | none | **ACTIVE** |

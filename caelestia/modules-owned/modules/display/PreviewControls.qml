...
StyledRect {
    ...
    readonly property string workspacePath: StandardPaths.writableLocation(StandardPaths.HomeLocation) + "/.local/bin/caerice-display-workspaces"
        ...
        stdout: StdioCollector {
            onStreamFinished: {
                try {
                    ...
                    else if (!action.running && !workspaceSync.running && !(root.lastResult?.confirmed ?? false) && !(root.lastResult?.persisted ?? false)) root.statusText = qsTr("No active preview");
        ...
        stdout: StdioCollector {
            onStreamFinished: {
                try {
                    ...
                    root.statusText = parsed?.ok
                        ? qsTr("Layout + workspace ranges persisted · 1–10 primary · 11–20 secondary")
                        : qsTr("Layout persisted; workspace policy needs review: %1").arg(parsed?.error ?? qsTr("unknown error"));
                } catch (error) { root.statusText = qsTr("Layout persisted; workspace policy returned invalid JSON"); }
            }
        }
    }
                ...
                try {
                    ...
                    if (root.actionPath === root.persistPath) {
                        if (parsed?.ok && parsed?.persisted) {
                            root.statusText = qsTr("Layout persisted · syncing workspace ranges…");
                            workspaceSync.command = [root.workspacePath, "sync", "--candidate", root.confirmedCandidateJson];
                            workspaceSync.running = true;
                        } else root.statusText = parsed?.error ?? qsTr("Persistence failed and was rolled back");
                    } else if (parsed?.ok && parsed?.preview) root.statusText = qsTr("Preview applied · Keep before the countdown expires");
                    ...
                    } else if (parsed?.ok && parsed?.reverted) { root.confirmedCandidateJson = ""; root.statusText = qsTr("Previous layout restored"); }
                    else if (!(parsed?.ok ?? false)) { root.confirmedCandidateJson = ""; root.statusText = parsed?.error ?? qsTr("Display action failed"); }
                } catch (error) { root.confirmedCandidateJson = ""; root.statusText = qsTr("Display action returned invalid JSON"); }
                root.refresh();
            }
        }
    }
    ...
    Column {
        ...
        Row {
            ...
            StyledRect { width:(parent.width-21)/4; height:42; radius:Tokens.rounding.large; color:root.active?Colours.palette.m3surfaceContainerHighest:Colours.palette.m3primaryContainer; enabled:!root.active&&!action.running&&!workspaceSync.running; opacity:enabled?1:0.55; StateLayer{radius:parent.radius;onClicked:root.startPreview()}; StyledText{anchors.centerIn:parent;text:qsTr("Preview");color:root.active?Colours.palette.m3onSurfaceVariant:Colours.palette.m3onPrimaryContainer;font:Tokens.font.label.medium} }
            ...
            StyledRect { width:(parent.width-21)/4; height:42; radius:Tokens.rounding.large; color:root.canPersist?Colours.palette.m3tertiaryContainer:Colours.palette.m3surfaceContainerHighest; enabled:root.canPersist&&!action.running&&!workspaceSync.running; opacity:enabled?1:0.55; StateLayer{radius:parent.radius;onClicked:root.persist()}; StyledText{anchors.centerIn:parent;text:qsTr("Save");color:root.canPersist?Colours.palette.m3onTertiaryContainer:Colours.palette.m3outline;font:Tokens.font.label.medium} }

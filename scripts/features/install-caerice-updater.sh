...
STAGE="$BACKUP/stage"
HELPERS=(caerice-upstream-audit caerice-updater caerice-updater-commit-base)
...
[[ -f "$SRC/UpdaterController.qml" ]] || { echo "ERROR: falta UpdaterController.qml" >&2; exit 3; }
for qml in "$SRC/updater/"*.qml; do [[ -f "$qml" ]] || { echo "ERROR: falta $qml" >&2; exit 3; }; done
for helper in "${HELPERS[@]}"; do [[ -f "$REPO/caelestia/bin/$helper" ]] || { echo "ERROR: falta $helper" >&2; exit 3; }; done
[[ -f "$REPO/scripts/features/validate-caerice-updater.py" ]] || { echo "ERROR: falta updater validator" >&2; exit 3; }
...
mkdir -p "$HOME/.local/bin"; for helper in "${HELPERS[@]}"; do install -m 0755 "$REPO/caelestia/bin/$helper" "$HOME/.local/bin/$helper"; done
hyprctl reload >/dev/null
echo "CaeRice Updater instalado. Backup: $BACKUP"; echo "Pipeline: Discover -> Fetch -> Test -> separate package update -> Snapshot -> Apply -> Verify -> Commit base/Rollback"; echo "Abre con Super+Shift+U o: qs -c caelestia ipc call updater open"

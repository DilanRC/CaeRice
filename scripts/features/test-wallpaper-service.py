...
service = (repo / "caelestia/services-owned/services/Wallpapers.qml").read_text()

for legacy in ("Caelestia", "qs.services", "Searcher", "FileSystemModel", "Colours.", "Paths."):
    assert legacy not in service, legacy
for contract in ("cortetsu/wallpaper/path.txt", "function query", "function preview", "previewGeneration", "cortetsu-wallpaper-select", "cortetsu-wallpaper-colours"):

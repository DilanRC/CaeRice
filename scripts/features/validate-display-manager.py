...
qml=['Wrapper.qml','Content.qml','Editor.qml','PreviewControls.qml','DisplayPresets.qml','DisplayCapabilities.qml']
...
for needle in ['Editor','PreviewControls','DisplayPresets','DisplayCapabilities']: req(needle in content,f'Content missing {needle}')
...
if probe:
 ...
 if monitors:
  m=monitors[0]; modes=m.get('available_modes',[]) if isinstance(m.get('available_modes'),list) else []; current=modes[0] if modes else 'preferred'
...
if not errors:
 ...
 ws=run_json([sys.executable,str(BIN/'caerice-display-workspaces'),'status'],'workspaces'); req('managed' in ws,'workspace status missing managed')
...
print('status: OK'); print('dry run + timed preview + exact-candidate Save: covered'); print('named layouts + workspace ranges: covered'); print('VRR/HDR/max-bpc capability reporting: covered; unknown is non-authoritative')
